...
class Game:
    ...
    def spawn_insects(self):
        ...
        if t > self.insects_spawn_timer:
            ...
            # tạo ra một con ong sau mỗi lần gọi hàm spawn_insects
            
            # increase the probability that the insect will be a bee over time
            # do đó, sau mỗi lần gọi hàm spawn_insects, xác suất để sinh ra một con ong sẽ tăng dần
            # nếu ta giảm từ 100 xuống 50, thì sau 1 phần trăm thời gian chơi, xác suất sinh ra ong sẽ tăng lên 50%
            nb = (GAME_DURATION-self.time_left)/GAME_DURATION * 100  / 2  # increase from 0 to 50 during all  the game (linear)
            if GAME_DURATION*0.75 < self.time_left < GAME_DURATION*0.9:
